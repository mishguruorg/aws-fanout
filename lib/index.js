...
import { compose, curry, map, without } from 'ramda'
...
const subscribeQueueToTopics = async (credentials, topicNames, queueName, deadLetterQueue, retries) => {
  ...
  try {
    ...
    const error = new Error(`Error setting queue permissions for queue: ${queueName} with topics:\n  - ${topicArnList.join('\n  - ')}`)
...
const subscribeQueueTopicsByTheirPrefix = async (credentials, topicNames, queueName, deadLetterQueue, retries) => {
  ...
  const topicsToSubscribe = without([topicPrefix], topicArnList) // TODO: proper method for removing ARN expressions

  await setQueuePermissions(credentials, [topicPrefix], queueInfo, deadLetterQueue, retries)

  return subscribeTopics(credentials, queueInfo.arn, topicsToSubscribe)
}
